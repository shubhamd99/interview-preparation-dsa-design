...
// beforeEach to do some setup before each test runs in order to avoid repetition.
beforeEach(() => {
  jest.spyOn(console, 'log'); // Creates a mock function similar to `jest.fn` but also tracks calls to object[methodName]
});

// beforeEach to do some setup after each test runs in order to avoid repetition.
afterEach(() => {
